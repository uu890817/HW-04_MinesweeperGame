...
#----------------新增CODE 1----------------#
    def left_click_actions(self, event):
        #----------------新增CODE 2----------------#
        print("點選位置參數:","(",self.x,",",self.y,")",self.is_mine,self.cell_count)     #DeBug用
        if self.is_mine == True and self.cell_count == 36:     #判斷第一次點即是否為地雷
            print("----觸發地雷重置----")                       #DeBug用
            self.reset()                                       #歸零地雷
            Cell.randomize_mines()                             #重新排放地雷
            self.left_click_actions(event)                     #模擬滑鼠左鍵點選
            return                                             #離開點選事件
        print(event)  #DeBug用
        #----------------新增CODE 2----------------#
